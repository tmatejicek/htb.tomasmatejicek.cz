...
Bitlab není o zranitelnosti v GitLabu samotném, ale o tom, co všechno útočník získá z příliš otevřeného vývojového prostředí. Veřejně přístupný GitLab, nalezené přihlašovací údaje `clave / 11des0081x` a dostupné repozitáře otevřou cestu ke zdrojovým kódům, databázovým konfiguracím i aplikaci, která se nasazuje na web. Širší bezpečnostní vzorec téhle situace popisuji i v článku [Repozitář, historie konfigurace a deployment trust](/techniky/repozitar-historie-konfigurace-a-deployment-trust).

Nejdůležitější není samotné přihlášení do GitLabu, ale schopnost změnit kód tak, aby si aplikace sama otevřela shell. Root část pak dobře ukazuje jiný druh chyby: administrativní `sudo git pull` spuštěný nad repozitářem, jehož obsah i hooky může ovlivnit útočník. Z pohledu řetězce jde zároveň o další variantu problému [Zápis do prostoru, který se pak vykoná nebo použije pro autentizaci](/techniky/zapis-do-prostoru-ktery-se-pak-vykona-nebo-pouzije-pro-autentizaci).
...
Lokálně je nebezpečný příkaz `sudo git pull`. Pokud může útočník zapisovat do pracovního adresáře a do `.git/hooks`, stačí přidat pomocný skript a nakopírovat ho i na hooky, které se spouštějí během pull nebo merge. Je to přesně ten případ, kdy privilegovaná automatika důvěřuje writable repozitáři.
```bash
