...
Share `kanban` byl čitelný i anonymně, což je důležité, protože právě v něm ležela lokální databáze nástroje PortableKanban. Praktickou roli rychlé mapy SMB share a jejich obsahu rozebírám i v článku [Smbmap](/nastroje/smbmap):
...
Pro .NET remoting dává smysl použít `ysoserial` a následně payload odeslat pomocí `ExploitRemotingService`. Princip je jednoduchý: aplikace přijme serializovaný objekt a během deserializace spustí dodaný příkaz. Praktickou roli tohoto typu nástroje rozebírám i v článku [ysoserial](/nastroje/ysoserial).
