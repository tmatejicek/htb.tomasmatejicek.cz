...
Anubis je hlavně o laterálním pohybu v doméně a o zneužití Active Directory Certificate Services. První vstup vede přes interní software portal a odchyt NTLM autentizace, ale skutečný dopad se ukáže až ve chvíli, kdy se podaří upravit certifikační šablonu a vystavit certifikát pro `Administrator`. Stejný princip rozebírám obecněji i v článku [AD CS a certifikáty jako cesta k Administratorovi](/techniky/ad-cs-a-certifikaty-jako-cesta-k-administratorovi).
...
Praktický základ takového úvodního průzkumu popisuji i v článku [Nmap](/nastroje/nmap).
...
Tady se opírám o zneužití webového endpointu pro spuštění příkazů (ASP payload). Pro stabilnější přístup jsem použil reverzní SOCKS tunel přes `chisel`. Praktickou roli tohoto tunelovacího nástroje rozebírám i v článku [Chisel](/nastroje/chisel).
...
Následně přes proxy vyvolám požadavek na interním instalačním endpointu, který vede na odchyt NTLMv2. Praktickou roli tohoto mostu rozebírám i v článku [Proxychains](/nastroje/proxychains). Širší logiku vynucené autentizace a následného zachycení popisuji i v technickém textu [NTLM coercion a vynucená autentizace](/techniky/ntlm-coercion-a-vynucena-autentizace).
...
Praktickou roli offline crackingu v podobných situacích shrnuji i v článku [John the Ripper](/nastroje/john-the-ripper).
...
Pro další posun je důležitý obsah `CertEnroll` a šablony certifikátů. Tady už se řetězec láme z obecného AD footholdu do konkrétního AD CS problému.
...
Vytvořím CSR s UPN `administrator@windcorp.htb` a požádám CA o certifikát. Praktickou roli tohoto kroku rozebírám i v článku [OpenSSL](/nastroje/openssl).
