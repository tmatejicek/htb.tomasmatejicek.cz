...
Tahle kombinace se dobře doplňuje hlavně s články [Špatně vystavené datové, storage a cloud-like služby](/techniky/spatne-vystavene-datove-storage-a-cloud-like-sluzby), [Container boundary mistakes: bind mounty, `docker exec`, `runc`, `privileged`](/techniky/container-boundary-mistakes-bind-mounty-docker-exec-runc-privileged) a [`sudo` nad package, backup a container nástroji](/techniky/sudo-nad-package-backup-a-container-nastroji).
...
Je to učebnicový případ článku [`sudo` nad package, backup a container nástroji](/techniky/sudo-nad-package-backup-a-container-nastroji): root se tu nedostává přes shell v `sudoers`, ale přes privilegovaný backup nástroj, který ochotně vyexportuje cizí data na útočníkem zvolený backend.
