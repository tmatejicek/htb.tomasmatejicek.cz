...
Je to dobrý stroj pro vysvětlení laterálního pohybu v rámci jednoho hostu: `www-data` nestačí, `kyle` nestačí a teprve kombinace skupin `filter` a `management` otevře cestu až k rootovi. Zároveň je to pěkný příklad kombinace [password reuse mezi aplikací a SSH](/techniky/password-reuse-a-rozpad-hranic-mezi-aplikaci-ssh-winrm-a-admin-nastroji) a [zápisu do důvěryhodného workflow](/techniky/zapis-do-prostoru-ktery-se-pak-vykona-nebo-pouzije-pro-autentizaci).
