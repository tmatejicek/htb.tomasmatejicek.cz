...
Praktickou roli podobných rychlých DNS dotazů rozebírám i v článku [host a dig](/nastroje/host-a-dig).
...
Dalším krokem bylo prolomení hashe. Praktickou roli offline crackingu v podobných řetězcích rozebírám i v článku [Hashcat](/nastroje/hashcat):
...
Tím vznikl první stabilní shell a bylo možné potvrdit `user.txt`. Důležitější než samotný flag ale byla lokální enumerace. Přes [enum4linux](/nastroje/enum4linux) bylo vidět, že ve skupině `Remote Management Users` je i servisní účet `svc_loanmgr`, tedy další kandidát na přímý management přístup.
...
Účet `svc_loanmgr` měl dostatečná oprávnění na to, aby `secretsdump.py` získal doménové hashe včetně `Administrator`. Praktický kontext podobných utilit rozebírám i v článku [Impacket pro relay, tajemství a delegovaná práva](/nastroje/impacket-pro-relay-tajemstvi-a-delegovana-prava).
