...
U Schooled není hlavní hodnota v jednom efektním kroku, ale ve vazbě mezi `moodle.schooled.htb`, Moodle a jeho pluginy a FreeBSD balíčky.
...
- Z hlediska rozhodování bylo nejdůležitější správně přečíst vazbu mezi `moodle.schooled.htb`, Moodle a jeho pluginy a FreeBSD balíčky.
...
- První obranná lekce míří na `moodle.schooled.htb`, Moodle a jeho pluginy a FreeBSD balíčky. Platformy typu Moodle je potřeba záplatovat i na úrovni pluginů a pomocných komponent; drobný webový modul často otevírá plnohodnotné spuštění kódu.
...
- Třetí lekce připomíná riziko, které v praxi představuje příliš široká `sudo` oprávnění. Pravidla `sudo`, která dovolují instalovat balíčky nebo spouštět obslužné nástroje s uživatelským vstupem, je potřeba omezit na pevné příkazy bez možnosti podvrhnout obsah.
