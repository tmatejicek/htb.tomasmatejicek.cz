...
Na FreeBSD je to kritické, protože balíček může obsahovat vlastní `+PRE_INSTALL` skript. Stačilo tedy připravit minimální balíček s instalačním hookem. Obecnější bezpečnostní význam takového `sudo` pravidla rozebírám i v článku [`sudo` nad package, backup a container nástroji](/techniky/sudo-nad-package-backup-a-container-nastroji):
