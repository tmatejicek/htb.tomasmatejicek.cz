...
Delivery je v téhle podobě nejzajímavější hlavně druhou půlkou řetězce. Jakmile je k dispozici nízko privilegovaný přístup `maildeliverer:Youve_G0t_Mail!`, systém začne dávat smysl teprve při čtení lokálních konfigurací OsTicketu a Mattermostu. Přesně tenhle způsob práce s provozními artefakty rozebírám i v článku [Metadata, logy, incidentní a forenzní artefakty jako zdroj přístupů](/techniky/metadata-logy-incidentni-a-forenzni-artefakty-jako-zdroj-pristupu).

Root tady nevzniká přes kernelový exploit ani sudo misconfiguraci. Rozhodující je, že z konfigurace Mattermostu vypadne silná nápověda `Crack_The_MM_Admin_PW`, vedle ní leží bcrypt hash Mattermost admin účtu `root` a v prostředí už je známý seed `PleaseSubscribe!`. Z toho se stane čistě offline útok na heslo a následné `su root`. Ve výsledku jde znovu o formu reuse mezi aplikační a systémovou vrstvou.
...
Tady má offline cracking jasný smysl. Útočník už neháda náhodné heslo, ale kombinuje konkrétní seed `PleaseSubscribe!` s nápovědou `Crack_The_MM_Admin_PW`. Z `best64.rule` tak vznikne malý, ale velmi relevantní slovník a hash padne na `PleaseSubscribe!21`. Praktickou stránku takového útoku rozebírám i v článku [Hashcat](/nastroje/hashcat).
```bash
cat rule
cat hash
...
Jakmile je hash cracknutý, nejde už o další exploitační techniku, ale o prosté ověření reuse hesla na lokální root účet. To je důležitý rozdíl: slabinou zde není `su`, ale kvalita a předvídatelnost hesla v prostředí, kde už útočník zná tematický seed. Přesně tenhle posun od aplikační nápovědy k systémovému účtu shrnuji i v článku [Password reuse a rozpad hranic mezi aplikací, SSH, WinRM a admin nástroji](/techniky/password-reuse-a-rozpad-hranic-mezi-aplikaci-ssh-winrm-a-admin-nastroji).
```bash
su root
cat root.txt
```
