...
Root tady nevzniká přes kernelový exploit ani sudo misconfiguraci. Rozhodující je, že z konfigurace Mattermostu vypadne silná nápověda `Crack_The_MM_Admin_PW`, vedle ní leží bcrypt hash účtu `root` a v prostředí už je známý seed `PleaseSubscribe!`. Z toho se stane čistě offline útok na heslo a následné `su root`.
...
Po přihlášení pod `maildeliverer` dává smysl číst hlavně lokální konfigurace. `/etc/passwd` potvrdí, že vedle `maildeliverer` běží i uživatel `mattermost`, OsTicket konfigurace vydá heslo `ost_user` a Mattermost config rovnou prozradí MySQL připojení i text `Crack_The_MM_Admin_PW`. Současně se objeví bcrypt hash účtu `root`.
```text
