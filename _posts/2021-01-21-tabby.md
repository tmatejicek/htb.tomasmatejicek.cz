...
Tabby dobře ukazuje, že průlom často nezačíná jedním exploitem, ale kombinací signálů jako Tomcat a jeho administrační vrstva, Apache a SSH.

Praktická část pak stojí na tom, jak se tyto zjištěné vazby promění v stabilní uživatelský přístup opřený o `news.php` a `tomcat-users.xml` a jak je po user části využitelná lokální enumeraci po získání shellu.
...
- User fáze se opírala o stabilní uživatelský přístup opřený o `news.php` a `tomcat-users.xml`, takže přístup byl reprodukovatelný a ne jen jednorázový.
- Finální kontrolu nad systémem otevřela až mechanika typu lokální enumerace po získání shellu.

## Co si odnést do praxe

- První obranná lekce míří na Tomcat a jeho administrační vrstva, Apache a SSH. Zálohy typu `backup.zip`, exporty registru a soubory `ntds.dit` je nutné chránit jako produkční tajemství; jejich únik znamená zkratku k účtům i hashům.
- Druhá lekce je o tom, jak rychle se ze zjištění stane stabilní uživatelský přístup opřený o `news.php` a `tomcat-users.xml`. Jakmile se v prostředí objeví použitelný klíč, heslo nebo token, je potřeba předpokládat okamžitý pivot na stabilní shell; obrana proto stojí na segmentaci a oddělení přístupů mezi službami.
