...
Patents je dobrý příklad stroje, kde o směru útoku nerozhodne první payload, ale kvalita reconu. Veřejně přístupné release notes, `.DS_Store` a `composer` metadata tady velmi rychle prozradí, že aplikace experimentuje s `include`, XML entitami a vlastním dokumentovým backendem `gears/pdf`.

Řetězec je přitom rozdělený do dvou zcela odlišných částí. User foothold vzniká ve webové vrstvě přes `convert.php` a zpracování dokumentů. Root část už naopak patří samostatné službě na `8888/tcp`, která používá vlastní protokol `LFM` a vlastní memory corruption chybu.
