...
Na Cascade je nejzajímavější, jak se propojí SMB sdílení, Kerberos a LDAP.
...
- Rozhodující bylo postupné řetězení účtů a hesel přes SMB a AD enumeraci, zejména přechod od `r.thompson` k `s.smith`.
- User fáze se opírala o první WinRM přístup pod `s.smith`, takže šlo o ověřitelný doménový foothold.
- Poslední krok pak otevřel další reuse tajemství směrem k `ArkSvc`, ne samostatný nový exploit.
