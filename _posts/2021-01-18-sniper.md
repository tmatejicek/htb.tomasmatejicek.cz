...
Sniper je dobrý příklad Windows webu, kde první shell nevzniká přes SQL injection ani přes klasický upload, ale přes nebezpečné includování souborů z parametru. Jakmile aplikace na IIS přijme cestu k PHP souboru bez omezení, dá se na Windows snadno zneužít i UNC cesta na útočníkův SMB share.

Druhá část stroje je stejně poučná: shell jako `NT AUTHORITY\\IUSR` ještě neznamená user část. Tu otevřela až lokální konfigurace webu a reuse databázového hesla pro účet `Chris`. Root pak stojí na workflow kolem `.chm` dokumentů, které na hostu zpracovával privilegovaný proces.
