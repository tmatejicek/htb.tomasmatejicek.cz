...
Omni je netypický Windows stroj postavený na Windows IoT Core a jeho `Windows Device Portal`. První zvláštnost je, že na `8080/tcp` neběží běžný web, ale zařízení se správním rozhraním. Druhá je, že samotné flagy nejsou uložené jako text, ale jako PowerShell `CliXml` objekty s credential payloadem.

Stroj je dobrý hlavně jako připomínka, že embedded a IoT varianty Windows mají jinou útočnou plochu než klasický server. Tady nedává smysl hledat SMB nebo WinRM; hlavní roli hraje Device Portal a možnost spouštět příkazy přes Sirep.
...
Na Omni funguje `SirepRAT`, který umí přes Sirep rozhraní spouštět příkazy na zařízení i bez běžného interaktivního shellu. Nejprve je rozumné ověřit, že spojení funguje a že příkazy běží v kontextu přihlášeného uživatele.
```text
...
Jakmile je potvrzené vzdálené spouštění, nejpraktičtější je stáhnout si na zařízení `nc64.exe` a udělat reverzní shell.
```text
...
Tohle je rozhodující bridge krok. Místo další eskalace stačí zneužít už existující mechanismus správy zařízení a přihlásit se do Device Portalu jako `administrator`.
...
- První praktický přístup otevírá `SirepRAT`, ale skutečný zlom přinese až skrytý `r.bat`, který resetuje heslo `administrator`.
