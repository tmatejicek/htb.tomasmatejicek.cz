...
Druhá polovina je čistě lokální Linux detail. `pspy` ukáže, že root při přihlášení volá `run-parts` bez absolutní cesty, takže rozhoduje obsah `$PATH`, ne další webová zranitelnost. To je přesně ten vzorec, který rozebírám i v článku [PATH, PYTHONPATH a wrapper hijack](/techniky/path-pythonpath-a-wrapper-hijack).
