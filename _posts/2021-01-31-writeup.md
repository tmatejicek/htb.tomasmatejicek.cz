...
V draftu je zachycen přechod od webové enumerace k reálnému uživatelskému přístupu. Klíčová indicie byla zmínka o podadresáři nebo sekci `writeup`, kde běžel zranitelný CMS Made Simple. SQL injection umožnila získat uživatelské jméno a hash; ten podle poznámek nevedl k přihlášení do administrace, ale fungoval pro SSH, což dává smysl jako oddělení webového a systémového přístupu.
