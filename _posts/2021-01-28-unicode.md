...
Veřejné write-upy právě tímto způsobem získávají administrátorský přístup do interní části aplikace. Odtud už vedla cesta ke zdrojovým kódům nebo konfiguračním datům, ve kterých byly SSH přihlašovací údaje pro účet `code`.
...
Veřejné write-upy toho využívají tak, že `curl` přimějí zapsat útočníkem připravený obsah do `/bin/bash`, nastaví na něj SUID bit a následně použijí `bash -p` pro root shell.
