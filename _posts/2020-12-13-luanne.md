...
U Luanne není hlavní hodnota v jednom efektním kroku, ale ve vazbě mezi webová aplikace v PHP, nginx a SSH.

Článek dává smysl číst hlavně jako rozbor rozhodování: proč právě tyto stopy vedou k SSH se získaným soukromým klíčem a proč po získání shellu dává smysl řešit lokální enumeraci po získání shellu.
...
- Počáteční průzkum se z obecné enumerace změnil v použitelný směr teprve po propojení indicií jako webová aplikace v PHP, nginx a SSH.
...
- V tomhle článku se první slabé místo otevřelo přes webová aplikace v PHP, nginx a SSH. Webová vrstva nesmí publikovat víc, než je nezbytné; vedlejší vhost, debug endpoint nebo zapomenutý soubor často odhalí skutečný vstup do řetězce.
