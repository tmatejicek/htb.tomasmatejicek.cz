...
Přesně pro podobnou situaci má smysl [Wfuzz](/nastroje/wfuzz): hlavní web vrací jen minimum, ale `Host` hlavička odhalí další větve aplikace, na kterých teprve leží skutečný foothold.
...
To je přesně ten typ vývojového artefaktu, který řeším v článku [Otevřený .git, zálohy a vývojové artefakty jako první foothold](/techniky/otevreny-git-zalohy-a-vyvojove-artefakty-jako-prvni-foothold).
...
Detailněji tenhle pattern rozebírám v článku [JWT signing secret a claim abuse](/techniky/jwt-signing-secret-a-claim-abuse).
