...
BigHead dobře ukazuje, že průlom často nezačíná jedním exploitem, ale kombinací signálů jako `dev.bighead.htb`, `backup.zip` a doménová záloha a WordPress a jeho pluginy.

Praktická část pak stojí na tom, jak se tyto zjištěné vazby promění v stabilní uživatelský přístup opřený o `linkto.php` a `win-shell-local.php` a jak je po user části využitelná lokální enumeraci po získání shellu.
...
- Z hlediska rozhodování bylo nejdůležitější správně přečíst vazbu mezi `dev.bighead.htb`, `backup.zip` a doménová záloha a WordPress a jeho pluginy.
- K uživatelskému kontextu vedl konkrétní a ověřitelný krok: stabilní uživatelský přístup opřený o `linkto.php` a `win-shell-local.php`.
- Poslední část ukazuje, že po získání shellu rozhoduje hlavně to, jakou roli hraje lokální enumerace po získání shellu.

## Co si odnést do praxe

- První obranná lekce míří na `dev.bighead.htb`, `backup.zip` a doménová záloha a WordPress a jeho pluginy. Zálohy typu `backup.zip`, exporty registru a soubory `ntds.dit` je nutné chránit jako produkční tajemství; jejich únik znamená zkratku k účtům i hashům.
- Druhá lekce je o tom, jak rychle se ze zjištění stane stabilní uživatelský přístup opřený o `linkto.php` a `win-shell-local.php`. Jakmile se v prostředí objeví použitelný klíč, heslo nebo token, je potřeba předpokládat okamžitý pivot na stabilní shell; obrana proto stojí na segmentaci a oddělení přístupů mezi službami.
