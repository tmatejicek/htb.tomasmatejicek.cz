...
Ve webové vrstvě hledám neveřejné cesty, vývojové artefakty a chybně vystavené soubory, protože právě ty často prozradí technologii aplikace, interní workflow nebo přímo přístupové údaje. Praktickou logiku takové enumerace rozebírám i v článcích [Dirsearch](/nastroje/dirsearch) a [Gobuster](/nastroje/gobuster).
```bash
...
Offline analýza ukázala klasické stack overflow v `bHeadSvr.dll`, offset 72 bajtů a použitelný `JMP ESP`. Tím vzniká první shell na cílovém hostu a zároveň možnost číst lokální konfiguraci Windows služeb. Právě tenhle přechod od veřejné binárky k vlastnímu exploitu shrnuji i v článku [Memory corruption a binary exploitation v praxi](/techniky/memory-corruption-a-binary-exploitation-v-praxi).
```text
offset 72 bajtů
JMP ESP => f0125062
./BigHead-exploit.py
...
Praktickou roli `certutilu` jako downloaderu i pomocníka pro práci s artefakty rozebírám i v článku [certutil](/nastroje/certutil).
