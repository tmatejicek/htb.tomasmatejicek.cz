...
Secret je dobrý příklad toho, jak nebezpečné je spojení zdrojových kódů, historie Git repozitáře a slepé důvěry v JWT token. Samotný web na první pohled nepůsobil dramaticky, ale po stažení zdrojáků se ukázalo, že citlivé informace neleží jen v aktuální verzi aplikace, ale i v jejím historickém vývoji.

Celý řetězec pak stojí na dvou odlišných chybách. První je slabá práce s tajemstvím pro podpis JWT, druhá je command injection v endpointu `/api/logs`, která promění administrátorský token v shell jako `dasith`. Root část je naopak čistě lokální a ukazuje, proč je kombinace SUID programu a povolených core dumpů velmi riziková.
